...
void CTilesetPainter::Load(const json_value &rElement)
{
...
const char* CTilesetPainter::GetRuleSetName(int Index) const
{
		...
		for(int x = 0; x < Width; x++)
		{
			...
			for(int i = 0; i < pConf->m_aRules.size(); ++i)
			{
				...
				for(int j = 0; j < pConf->m_aRules[i].m_aConditions.size() && RespectRules; ++j)
				{
					...
					int CheckIndex = clamp((y+pCondition->m_Y), 0, Height-1)*Width+clamp((x+pCondition->m_X), 0, Width-1);
					...
					else
					{
						...
						{
							if(pTiles[CheckIndex].m_Index > 0 && pCondition->m_Value == CRuleCondition::EMPTY)
								RespectRules = false;

							if(pTiles[CheckIndex].m_Index == 0 && pCondition->m_Value == CRuleCondition::FULL)
								RespectRules = false;
						}
						else
						{
							if(pTiles[CheckIndex].m_Index != pCondition->m_Value)
								RespectRules = false;
						}
					}
				}
...
int IAutoTiler::CompareRules(const void *a, const void *b)
{
	CDoodadsPainter::CRule *ra = (CDoodadsPainter::CRule*)a;
	CDoodadsPainter::CRule *rb = (CDoodadsPainter::CRule*)b;

	if((ra->m_Location == CDoodadsPainter::CRule::FLOOR && rb->m_Location == CDoodadsPainter::CRule::FLOOR)
		|| (ra->m_Location == CDoodadsPainter::CRule::CEILING && rb->m_Location  == CDoodadsPainter::CRule::CEILING))
	{
	...
	else if(ra->m_Location == CDoodadsPainter::CRule::WALLS && rb->m_Location == CDoodadsPainter::CRule::WALLS)
	{
...
void CDoodadsPainter::Load(const json_value &rElement)
{
	for(unsigned i = 0; i < rElement.u.array.length; ++i)
	{
		if(rElement[i].u.object.length != 1)
...
const char* CDoodadsPainter::GetRuleSetName(int Index) const
{
	if(Index < 0 || Index >= m_aRuleSets.size())
		return "";
...
void CDoodadsPainter::AnalyzeGameLayer()
{
	...
	for(int y = 1; y < pLayer->m_Height-1; y++)
	{
		...
		{
			CTile *pTile = &(m_pGameTiles[y*pLayer->m_Width+x]);
			...
			// add a floor part
			if(pTile->m_Index == 1 && (m_pGameTiles[CheckIndex].m_Index == 0 || m_pGameTiles[CheckIndex].m_Index > ENTITY_OFFSET))
			{
				// create an new chain
...
void CDoodadsPainter::PlaceDoodads(CTile* pTiles, int Width, int Height, CRule *pRule, array<array<int> > *pPositions, int Amount, int LeftWall)
{
	...
	for(int f = 0; f < pPositions->size(); f++)
		for(int c = 0; c < (*pPositions)[f].size(); c += pRule->m_Size.x)
		{
			...
			if(RandomValue > 1 && !IAutoTiler::Random(RandomValue))
				continue;
