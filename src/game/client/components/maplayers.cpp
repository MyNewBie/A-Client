...
void CMapLayers::LoadBackgroundMap()
{
	...
	str_format(aBuf, sizeof(aBuf), "ui/themes/%s_%s.map", g_Config.m_ClMenuMap, (HourOfTheDay >= 6 && HourOfTheDay < 18 && !g_Config.m_ClMapForceNight) ? "day" : "night"); //d n
	if(!m_pMenuMap->Load(aBuf, m_pClient->Storage()))
	{
		...
		if(!m_pMenuMap->Load(aBuf, m_pClient->Storage()))
		{
			...
			str_format(aBuf, sizeof(aBuf), "ui/themes/%s_%s.map", g_Config.m_ClMenuMap, (HourOfTheDay >= 6 && HourOfTheDay < 18 && !g_Config.m_ClMapForceNight) ? "night" : "day"); //n d
			if(!m_pMenuMap->Load(aBuf, m_pClient->Storage()))
			{
...
void CMapLayers::EnvelopeEval(float TimeOffset, int Env, float *pChannels, void *pUser)
{
	...
	float Time = 0.0f;
	if(pThis->Client()->State() != IClient::STATE_OFFLINE)
	{
...
void CMapLayers::OnRender()
{
	...
	for(int g = 0; g < pLayers->NumGroups(); g++)
	{
		...
		for(int l = 0; l < pGroup->m_NumLayers; l++)
		{
			...
			if(!(pLayer->m_Flags&LAYERFLAG_DETAIL && !g_Config.m_GfxHighDetail && !IsGameLayer && (Client()->State() == IClient::STATE_ONLINE)))
			{
