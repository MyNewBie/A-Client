...
void CGameClient::OnInit()
{
	...
	m_pEntities->DelayedInit();

	OnReset();

	int64 End = time_get();
	char aBuf[256];
	str_format(aBuf, sizeof(aBuf), "initialization finished after %.2fms", ((End-Start)*1000)/(float)time_freq());
...
int CGameClient::IntersectCharacter(const vec2& HookPos, const vec2& NewPos, vec2 *pOutNewPos, int ownID)
{
	...
	for (int i=0; i < MAX_CLIENTS; i++)
	{
		...
		if (!cData.m_Active || i == ownID)
			continue;

		vec2 ClosestPoint = closest_point_on_line(HookPos, NewPos, Position);
		if(distance(Position, ClosestPoint) < 30.0f)
		{
